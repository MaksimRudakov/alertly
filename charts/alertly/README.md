...
| config.updates.commands | object | `{"enabled":false}` | Read-only chat-ops commands (/status) over the same long-poll loop and allowlists. Register the command in BotFather (/setcommands) for autocompletion. |
