...
| config.dedup.enabled | bool | `true` | Enable in-process dedup. |
| config.dedup.ttl | string | `"1h"` | Window during which a repeat delivery for the same (fingerprint, chat, status) is suppressed. |
